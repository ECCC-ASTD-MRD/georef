...
const char *TRef_InterpString[] = { "UNDEF","NEAREST","LINEAR","CUBIC","NORMALIZED_CONSERVATIVE","CONSERVATIVE","MAXIMUM","MINIMUM","SUM","AVERAGE","AVERAGE_VARIANCE","AVERAGE_SQUARE","NORMALIZED_COUNT","COUNT","VECTOR_AVERAGE","NOP","ACCUM","BUFFER","SUBNEAREST","SUBLINEAR", "WEIGHTINDEX",
   "FAST","WITHIN","INTERSECT","CENTROID","ALIASED","CONSERVATIVE","NORMALIZED_CONSERVATIVE","POINT_CONSERVATIVE","LENGTH_CONSERVATIVE","LENGTH_NORMALIZED_CONSERVATIVE","LENGTH_ALIASED",NULL };
...
void Georef_PrintOptions(TGeoOptions *Options) {
   ...
   Lib_Log(APP_LIBGEOREF,APP_ALWAYS,
        ...
        "Lookup table dim    : %i\n"
        "Ancilliary buffer   : %p\n",
         Options->Interp,Options->Extrap,Options->Combine,Options->Transform,
...
TGeoRef* GeoRef_Define(TGeoRef *Ref,int32_t NI,int32_t NJ,char* GRTYP,char* grref,int32_t IG1,int32_t IG2,int32_t IG3,int32_t IG4,double* AX,double* AY) {
   ...
   // No GRID_SUB means it's a subgrid definition so IGs are reference IGs (ex: Creation of YinYan subgrids)
   if (Ref==GRID_SUB) {
