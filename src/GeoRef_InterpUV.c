...
int32_t GeoRef_UV2WD(
    //! [in] Georeference
    TGeoRef * const Ref,
    //! [out] Destination speed values
    float * const spdout,
    //! [out] Destination direction values
    float * const wdout,
    //! [in] Source UU values
    const float * const uuin,
    //! [in] Source VV values
    const float * const vvin,
    //! [in] Latitude of wind value points (NULL, use geo-reference gridpoints)
    const double * const Lat,
    //! [in] Longitude of wind value points (NULL, use geo-reference gridpoints)
    const double * const Lon,
    //! [in] Number of values to convert (size of arrays)
    const int32_t Nb
) {
    //! \return FALSE (0) if operation failed, TRUE (1) otherwise
    ...
    if (!Lat || !Lon) GeoRef_CalcLL(Ref);
    const double * const lat = Lat ? Lat : Ref->Lat;
    const double * const lon = Lon ? Lon : Ref->Lon;
    ...
    switch (Ref->GRTYP[0]) {
        case 'E': {
            double * const lat_rot = (double *)(malloc(2 * Nb * sizeof(double)));
            double * const lon_rot = &lat_rot[Nb];
...
void c_ezgfwfllw8(
    ...
    double * const Lat,
    double * const Lon,
    double * const xlatingf,
    double * const xloningf,
    ...
    double * const uvcart = (double *)malloc(2 * 3 * npts * sizeof(double));
    double * const xyz = &uvcart[3 * npts];

    float r[3][3], ri[3][3], xlon1, xlat1, xlon2, xlat2;
    ...
    f77name(ez8_mxm)((const float * const)r, &trois, xyz, &trois, uvcart, &npts);
...
//! Rotate winds from a rotated coordinate system to a non-rotated one
void c_ezllwfgfw8(
    float * const uullout,
    float * const vvllout,
    //! [in] True latitudes
    ...
    const double * const xlatingf,
    //! [in] Longitudes on the rotated grid
    const double * const xloningf,
    const int32_t * const ni,
    const int32_t * const nj,
    const char * const grtyp,
    const int32_t * const ig1,
