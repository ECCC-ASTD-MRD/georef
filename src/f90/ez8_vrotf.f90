...
!
      subroutine ez8_vrotf(u, v, lonp, latp, lon, lat, ro, xyz, uvcart, ni, nj)
      implicit none
      integer ni, nj 
      real*8 lonp(ni,nj), lon(ni,nj), latp(ni,nj), lat(ni,nj), xyz(3,ni*nj), uvcart(3,ni*nj)
...
!                coordonnees tourne par rapport a la geographie reelle
      call ez8_cartauv(u, v, uvcart, lon, lat, ni, nj)
