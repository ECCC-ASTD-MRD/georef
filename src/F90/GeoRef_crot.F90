...
!> Compute the rotation matrix that allows the transformation from the spherical coordinate system to the rotated spherical coordinate system
subroutine GeoRef_crot( r, ri, lon1, lat1, lon2, lat2 )
    implicit none

    !> Rotation matrix from true to rotated coordinate system
    real, intent(out) :: r(3,3)
    !> Rotation matrix from rotated to true coordinate system
    real, intent(out) :: ri(3,3)
    ...
    call GeoRef_lac(xyz1, [lon1], [lat1], 1)
    call GeoRef_lac(xyz2, [lon2], [lat2], 1)
