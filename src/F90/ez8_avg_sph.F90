subroutine ez_avg_sph(zout, xx, yy, lats_dst, ni_dst, nj_dst, zin, ni_src, nj_src, extension)
    ...
    integer, intent(in) :: ni_src, nj_src, ni_dst, nj_dst
    real, dimension(ni_src, nj_src), intent(in) :: zin
    real, dimension(ni_dst, nj_dst), intent(out) :: zout
    real(kind = real64), dimension(ni_dst, nj_dst), intent(in) :: xx, yy
    real(kind = real64), dimension(nj_dst), intent(in) :: lats_dst
    ...
    integer low_bound_x, high_bound_x
    ...
    do j = 1, nj_dst
        amplif_lats(j) = real(1.0 / cos(lats_dst(j) * degre_a_radian))
    enddo
