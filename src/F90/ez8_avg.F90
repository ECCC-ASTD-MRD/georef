subroutine ez8_avg(zout, xx, yy, ni_dst, nj_dst, zin, ni_src, nj_src, extension)
    use iso_fortran_env, only: real64
    implicit none

    integer, intent(in) :: ni_src, nj_src, ni_dst, nj_dst
    real, intent(out) :: zout(ni_dst, nj_dst)
    real(kind = real64), intent(in) :: xx(ni_dst, nj_dst)
    real(kind = real64), intent(in) :: yy(ni_dst, nj_dst)
    real, intent(in) :: zin(ni_src, nj_src)
