...
def validate_interpolation(src_file, target_file):
    ...
    with fst24_file(src_file, "R") as f_src, fst24_file(target_file, "R") as f_target:
        ...
        print(
            f"[*] Succès {src_file:16s} -> {target_file:16s}"
            f"  Norme relative : {norm:.2e}"
            f"  Erreur Max     : {max_err:.2e}"
        )
        # print("-" * 40)
...
base_config = {"lons": np.array([0, 45]), "lats": np.array([0, 10]), "ni": 180, "nj": 90}
    ...
    grids_config = {
        ...
        "A2": {
            **base_config,
            ...
            "nj": 80,
            "grtyp": "A",
            "filename": "Grid_A2.fst",
            "label": "Lat-Lon Equidistante",
        },
