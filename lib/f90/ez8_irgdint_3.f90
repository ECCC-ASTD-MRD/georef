...
! */
      subroutine ez8_irgdint_3(zo,px,py,npts,z,ni,i1,i2,j1,j2,ax,ay,cx,cy,wrap,nodata)
      implicit none
!*******
...
!*******
      ...
      real(kind=8) ax(i1:i2),ay(j1:j2)
      real cx(i1:i2,6),cy(j1:j2,6)
      real z(i1:i2,j1:j2),nodata
!     
...
!  structure identique pour cy(j,1..6)
      ...
      integer i, j, n
      real(kind=8) a11,a12,a13,a14,a21,a22,a23,a24
      real(kind=8) a31,a32,a33,a34,a41,a42,a43,a44
      real(kind=8) b1,b2,b3,b4,b11,b12,b13,b14
      real x1,x2,x3,x4,y1,y2,y3,y4
      
      real(kind=8) fa, fa2, fa3, fa4
      real(kind=8) a1,a2,a3,a4,x,y,c1,c2,c3,c4,c5,c6
         ...
         if (defvalid(real(z1),nodata) .and. defvalid(real(z2),nodata) .and. defvalid(real(z3),nodata) .and. defvalid(real(z4),nodata)) then        
            ...
            if (defvalid(real(z1),nodata) .and. defvalid(real(z2),nodata) .and. defvalid(real(z3),nodata) .and. defvalid(real(z4),nodata)) then        
               ...
               if (defvalid(real(z1),nodata) .and. defvalid(real(z2),nodata) .and. defvalid(real(z3),nodata) .and. defvalid(real(z4),nodata)) then        
                  ...
                  a34 = fa4(dble(cx(i,1)),dble(cx(i,2)),dble(cx(i,3)),dble(cx(i,4)),    &
                     dble(cx(i,5)),dble(cx(i,6)),dble(z1),dble(z2),dble(z3),dble(z4))
                  ...
                  if (defvalid(real(z1),nodata) .and. defvalid(real(z2),nodata) .and. defvalid(real(z3),nodata) .and. defvalid(real(z4),nodata)) then        
                     ...
                     a44 = fa4(dble(cx(i,1)),dble(cx(i,2)),dble(cx(i,3)),dble(cx(i,4)),      &
                        dble(cx(i,5)),dble(cx(i,6)),dble(z1),dble(z2),dble(z3),dble(z4))
                     ...
                     b14 = fa4(dble(cy(j,1)),dble(cy(j,2)),dble(cy(j,3)),       &
                     dble(cy(j,4)),dble(cy(j,5)),dble(cy(j,6)),b1,b2,b3,b4)
