...
void OGM_GPCFromOGR(gpc_polygon *Poly,OGRGeometryH *Geom) {
   ...
   if (type==wkbMultiPolygon || type==wkbGeometryCollection) {
      ...
      Lib_Log(APP_LIBGEOREF,APP_ERROR,"%s: Unsupported geometry type\n",__func__);
   }
#else
   Lib_Log(APP_LIBGEOREF,APP_ERROR,"%s: Library not built with GPC\n",__func__);
#endif
}
      ...
      for(g=0;g<Poly->num_contours;g++) {
         if (Poly->hole[g]) {
            ...
            if (multi) {
               ...
               if (!in) {
                  Lib_Log(APP_LIBGEOREF,APP_ERROR,"%s: Found a hole without parent\n",__func__);
               }
            } else {
               OGR_G_AddGeometryDirectly(poly,ring);
            }
...
int OGM_Simplify(double Tolerance,OGRGeometryH Geom) {
   ...
   if ((n=OGM_ToVect3d(Geom,OGM_ARRAY0))>2) {
      ...
      if (!mk) {
         Lib_Log(APP_LIBGEOREF,APP_ERROR,"%s: Unable to allocate buffers\n",__func__);
         return(0);
      }
