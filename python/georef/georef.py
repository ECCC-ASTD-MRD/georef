...
class GeoRef:
    ...
    def __init__(
        self, ni: int, nj: int, grtyp: str, ig1: int, ig2: int, ig3: int, ig4: int, fst_file: fst24_file | None = None
    ) -> None:
    ...
    def intersect(self, other, boundary=False) -> Union[None, Tuple[int, int, int, int]]:
        ...
        val = _intersect(
            self._ptr, other._ptr, ctypes.byref(x0), ctypes.byref(y0), ctypes.byref(x1), ctypes.byref(y1), bd_val
        )
    ...
    def boundingbox(self, lat0: float, lon0: float, lat1: float, lon1: float) -> Tuple[float, float, float, float]:
        ...
        val = _boundingbox(
            self._ptr, lat0, lon0, lat1, lon1, ctypes.byref(i0), ctypes.byref(j0), ctypes.byref(i1), ctypes.byref(j1)
        )
    ...
    def uv2wd(self, uu_in, vv_in, lat, lon) -> Tuple[numpy.ndarray, numpy.ndarray]:
        ...
        """
        assert isinstance(uu_in, numpy.ndarray) and isinstance(vv_in, numpy.ndarray), GeoRefError(
            "uu_in and vv_in must be numpy arrays"
        )
    ...
    def wd2uv(self, spd_in, dir_in, lat, lon) -> Tuple[numpy.ndarray, numpy.ndarray]:
        ...
        Note:
            ...
            - Non-zero for failure (NULL references or conversion error)
        """
        assert isinstance(spd_in, numpy.ndarray) and isinstance(dir_in, numpy.ndarray), GeoRefError(
            "spd_in and dir_in must be numpy arrays"
        )
    ...
    def ll2xy(self, lat, lon, extrapolate=False) -> Tuple[numpy.ndarray, numpy.ndarray]:
        ...
        """
        assert isinstance(lat, numpy.ndarray) and isinstance(lon, numpy.ndarray), GeoRefError(
            "lat and lon must be numpy arrays"
        )
        npts = lat.size
    ...
    def xy2ll(self, x, y, extrapolate=False) -> Tuple[numpy.ndarray, numpy.ndarray]:
        ...
        Note:
            ...
            - 0 or negative value indicates complete failure
        """
        assert isinstance(x, numpy.ndarray) and isinstance(y, numpy.ndarray), GeoRefError(
...
class GeoSet:
    ...
    def read_fst(
        cls, ref_to: GeoRef, ref_from: GeoRef, interp_type: int, file: fst24_file
    ):  # -> GeoSet: (but not defined)
